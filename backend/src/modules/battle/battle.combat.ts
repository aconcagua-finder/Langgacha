...
import { INSPIRATION_BONUS, RANDOMIZATION_FACTOR } from "../../shared/constants.js";
import type { BattleCard, CombatTick } from "./battle.types.js";

export { applyConditionModifier, computeDamage, computeHp };

const rollCombatDamage = (baseDamage: number): number => {
  const minDamage = Math.max(1, Math.floor(baseDamage * (1 - RANDOMIZATION_FACTOR)));
  const maxDamage = Math.max(minDamage, Math.ceil(baseDamage * (1 + RANDOMIZATION_FACTOR)));
...
export const simulateCombat = (params: {
  ...
  while (playerHp > 0 && botHp > 0) {
    if (attacker === "player") {
      const damage = rollCombatDamage(computeDamage(playerAtk, params.bot.def, params.bot.hp));
      botHp = Math.round(botHp - damage);
      log.push({ attacker: "player", damage, targetHpAfter: Math.max(0, botHp) });
      attacker = "bot";
    } else {
      const damage = rollCombatDamage(
        computeDamage(params.bot.atk, params.player.def, params.player.hp),
      );
